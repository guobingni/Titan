...
public class TicketMasterAPI implements ExternalAPI {
	...
	private static final String API_KEY = "RMksIaMZKyYuoEq72rf2zyfKVJsPoQrr";

	/**
	 * Creates and sends a request to the TicketMaster API by term and location.
	 */
	// why json to item then to json
	// 很多信息不需要，简化，可以得到flatten form
	@Override
	public List<Item> search(double lat, double lon, String term) {
		...
		term = urlEncodeHelper(term);  // url cannot have space or other special character
		String query = String.format("apikey=%s&latlong=%s&keyword=%s", API_KEY, latlong, term);
		try {
			// (semantic) construct a connection (get method)
			HttpURLConnection connection = (HttpURLConnection) new URL(url + "?" + query).openConnection();
			connection.setRequestMethod("GET");
			// response 
			// call getresponsecode or getinputstream, a connection will be constructed (lazy operation)
	 ...
	 */
	// java application must have a main function
